...
/**
* Controlador encargado de manejar los precios desde la aplicacion web.
*
* @package         CodeIgniter
* @subpackage      Controllers
* @category        Controller
*/
class Price_web_controller extends CI_Controller {
    ...
    public function load_masive_price_view()
    {
        if ($this->session->userdata('user')) {
            $data["user"] = $this->session->userdata('user');
            $data["businesses"] = $this->Commerce_model->get_businesses();

            $this->load->view('menu_admin', $data);
            $this->load->view('load_masive_price_view', $data);
        } else {
            redirect(base_url());
        }
    }

    public function load_masive_price()
    {
        if ($this->session->userdata('user')) {
            $data["user"] = $this->session->userdata('user');
            ...
            if (!$this->upload->do_upload()) {
                ...
                $this->_load_prices($commerce, $file_path, $data);
    ...
    private function _load_prices($commerce, $file, $data)
    {
        if ($this->csvimport->get_array($file)) {
            ...
            foreach ($csv_array as $row) {
                if (isset($row['PRECIO']) && isset($row['CODIGO'])) {
                    $is_valid = TRUE;
                } else {
                    $is_valid = FALSE;
                }
            ...
            foreach ($csv_array as $row) {
                $product_exists = $this->Product_model->product_exists($row['CODIGO']);

                if (!$product_exists) {
                    $this->Product_model->register_product($row['CODIGO']);
                    $this->IS_NEW_PRODUCT = TRUE;
                }

                $this->Price_model->register_price($commerce, $user, $row['PRECIO'], $row['CODIGO']);
                $this->_calculate_price($commerce, $row['PRECIO'], $row['CODIGO']);
                $count++;
            }

            $data['success'] = "Se cargaron correctamente ".$count." precios";
            $data["businesses"] = $this->Commerce_model->get_businesses();
            $this->load->view('menu_admin', $data);
            $this->load->view('load_masive_price_view', $data);
        } else {
            $data['error'] = "No se pudieron cargar los precios";
            $data["businesses"] = $this->Commerce_model->get_businesses();
            $this->load->view('menu_admin', $data);
            $this->load->view('load_masive_price_view', $data);
        }
